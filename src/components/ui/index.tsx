...
export function Button({ variant = 'secondary', size = 'sm', loading, children, className, disabled, ...props }: ButtonProps) {
  ...
  return (
    <button className={cn('inline-flex items-center justify-center gap-1.5 font-medium rounded-md transition-all disabled:opacity-50 disabled:cursor-not-allowed', v[variant], s[size], className)} disabled={disabled ?? loading} {...props}>
...
// Badge
export function Badge({ children, color, bgColor, textColor, className }: { children: React.ReactNode; color?: string; bgColor?: string; textColor?: string; className?: string }) {
  return (
    <span className={cn('inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium', className)}
      style={{ backgroundColor: bgColor ?? '#F3F4F6', color: textColor ?? '#4B5563', border: `1px solid ${color ?? '#E5E7EB'}40` }}>
      {children}
    </span>
  )
}
...
// Avatar
export function Avatar({ name, size = 32, bg = '#2563EB' }: { name: string; size?: number; bg?: string }) {
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: bg, color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: Math.floor(size * 0.35), fontWeight: 600, flexShrink: 0 }}>
      {getInitials(name)}
    </div>
  )
}
...
export function Modal({ open, onClose, title, children, size = 'md' }: { open: boolean; onClose: () => void; title?: string; children: React.ReactNode; size?: 'sm' | 'md' | 'lg' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.4)' }} onClick={e => { if (e.target === e.currentTarget) onClose() }}>
      <div className={cn('bg-white rounded-xl shadow-xl w-full', widths[size])}>
        {title && (
