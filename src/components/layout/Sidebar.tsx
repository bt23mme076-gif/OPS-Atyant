...
export function Sidebar({ onNavigate }: SidebarProps) {
  ...
  return (
    <div className="flex flex-col flex-1 select-none overflow-y-auto">
      {/* Brand — hidden on mobile (shown in layout topbar instead) */}
      ...
      <Link
        href="/notifications"
        onClick={onNavigate}
        className="mx-3 mt-3 flex items-center gap-2.5 rounded-md px-3 py-2 text-white/60 hover:text-white hover:bg-white/8 transition-colors text-sm"
      >
        <Bell size={15} />
      ...
      <div className="border-t border-white/10 p-3">
        <div className="flex items-center gap-2.5 px-2 py-1.5 rounded-md hover:bg-white/8 cursor-pointer group">
          ...
          <button onClick={handleLogout} className="text-white/40 hover:text-red-400 transition-all md:opacity-0 md:group-hover:opacity-100" title="Logout">
            <LogOut size={13} />
          </button>
        </div>
      </div>
